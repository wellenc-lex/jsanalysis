    ...
    for url in urls:
                ...
                try:
                    ...
                    if args.output == 'cli':
                        ...
                        for endpoint2 in new_endpoints:
                            ...
                            header = "<div><a href='%s'class='textblock'>%s" % (
                                html.escape(url),html.escape(url)
                            )
                            body = "</a><div class='containerjs'>%s</div></div>" % html.escape(
                                endpoint2["context"]
                            )
                            body = body.replace(
        ...
        if args.output == 'cli':
            ...
            for endpoint in endpoints:
                ...
                header = "<div><b class='textblock'>%s" % (
                    html.escape(url),
                )
                body = "</b><div class='containerjs'>%s</div></div>" % html.escape(
                    endpoint["context"]
                )
                body = body.replace(
